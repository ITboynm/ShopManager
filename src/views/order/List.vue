  ...
  <div id="goods" :style="{ height: `${$windowHeight - 60 - 44 - 24}px` }">
    ...
    <div class="goods-main">
      ...
      <Search v-model="queryform" :rules="queryRules" @search="handleQuery" @reset="getData(pager)"
        ...
        <SearchItem :prop="'no'">
          <el-input v-model="queryform.no" size="small" placeholder="订单编号"></el-input>
        </SearchItem>
        <template #moreSearch>
          <SearchItem label="收货人" prop="name">
            <el-input v-model="queryform.name" placeholder="收货人" clearable></el-input>
          </SearchItem>
          ...
          <SearchItem label="开始时间" prop="starttime">
            <el-date-picker v-model="queryform.starttime" type="date" placeholder="开始日期" style="width: 90%;"
              value-format="YYYY-MM-DD" />
          </SearchItem>
          <SearchItem label="结束时间" prop="endtime" class="mb-2">
            <el-date-picker v-model="queryform.endtime" type="date" placeholder="结束日期" style="width: 90%;"
              value-format="YYYY-MM-DD" />
          </SearchItem>
        </template>
      </Search>
      <el-card shadow="never" class="border-0 relative" :style="{ height: `${$windowHeight - 60 - 44 - 22 - 78}px` }">
        <!-- 新增和刷新 -->
        <ListHeader layout="refresh,download" @refresh="getData(pager)" @download="handleExportExcel">
          <el-popconfirm v-if="queryform.tab != 'delete'" title="是否删除选中项?" confirm-button-text="确认"
            cancel-button-text="取消" width="158px" @confirm="handleDelete(multiSelectionIds)">
            <template #reference>
              <el-button type="danger" size="small" icon="Delete">批量删除</el-button>
            </template>
          </el-popconfirm>
          <div v-else class="inline-block ml-[12px]">
            <el-popconfirm title="是否彻底删除选中项?" confirm-button-text="确认" cancel-button-text="取消" width="178px"
              @confirm="handleShopStatus(multiSelectionIds, 'destroy')">
              <template #reference>
                <el-button type="danger" size="small" icon="Delete">彻底删除</el-button>
              </template>
            </el-popconfirm>
            <el-popconfirm title="是否恢复选中项?" confirm-button-text="确认" cancel-button-text="取消" width="158px"
              @confirm="handleShopStatus(multiSelectionIds, 'res')">
              <template #reference>
                <el-button type="warning" size="small" icon="RefreshLeft">恢复商品</el-button>
              </template>
            </el-popconfirm>
          </div>
        ...
        <el-table :data="tableData" stripe style="width: 100%" v-loading="loading" ref="multipleTableRef"
            ...
            <template #default="{ row }">
              <div>
                ...
                <div class="flex py-2 items-center" v-for="(item, index) in row.order_items" :key="index">
                  <el-image
                    :src="item.goods_item ? item.goods_item.cover : 'https://cube.elemecdn.com/e/fd/0fc7d20532fdaf769a25683617711png.png'"
                    fit="cover" :lazy="true" style="width: 30px;height: 30px;"></el-image>
          ...
          <el-table-column label="操作" align="center">
            <template #default="{ row }">
              ...
              <el-button v-if="queryform.tab === 'noship'" class="px-1" type="primary" size="small" text
                @click="shipOrder(row)">订单发货</el-button>
              <el-button v-if="queryform.tab === 'refunding'" class="px-1" type="primary" size="small" text
      ...
      <FormDrawer title="填写快递单号" ref="formDrawerRef" @submit="handleSubmit" @reset="resetShipForm">
        <el-form :model="shipForm" ref="DrawerRef" label-width="100px" :inline="false">
...
const resetCreateForm = () => {
  queryform.starttime = queryform.endtime = queryform.name = queryform.phone = null
}
...
const handleRefund = (id, agree) => {
  (agree ? showModal("是否要同意该订单退款?") : showPromptModal("请输入拒绝的理由"))
    .then(async ({ value }) => {
      let data = { agree }
      if (!agree) {
        data.disagree_reason = value
      }
      const res = await fetchData(orderApi.handleRefund, { fetchId: id, fetchContent: data }, '操作')
...
const handleSubmit = async () => {
  ...
  const res = await fetchData(orderApi.orderShip, {
