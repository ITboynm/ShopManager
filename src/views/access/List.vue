<template>
	<div id="">
		<el-card shadow="never" class="border-0 relative"
			...
			<div class="treeBox" :style="{ height: `${$windowHeight.value - 60 - 44 - 22 - 70 - 55}px` }">
				<el-tree :data="tableData" :props="{ label: 'name', children: 'child' }" class="custom-tree-node"
					@node-click="handleNodeClick" v-loading="loading" node-key="id"
						...
						<div class="flex items-center w-[100%]">
							...
							<div class="ml-auto">
								<el-switch :model-value="data.status" :active-value="1" :inactive-value="0"
									@change="handleStatusChange($event, data)" @click.native.stop
								...
								<el-popconfirm :title="data.menu ? '是否删除该菜单?' : '是否删除该权限?'" confirm-button-text="确认"
									...
									<template #reference>
										<el-button text type="danger" size="small" @click.native.stop
