<template>
    <div v-loading="loading" class="bg-white p-4 rounded" :style="{ height: `${$windowHeight - 60 - 44 - 24}px` }">
        ...
        <FormDrawer ref="formDrawerRef" title="配置" @submit="submit">
            ...
            <el-form v-else :model="form" label-width="80px">
                ...
                <el-form-item label="cert_client">
                    <el-upload :action="sysconfigApi.uploadAction" :headers="{
                        token
                    }" accept=".pem" :limit="1" :on-success="uploadClientSuccess">
                ...
                <el-form-item label="cert_key">
                    <el-upload :action="sysconfigApi.uploadAction" :headers="{
                        token
                    }" accept=".pem" :limit="1" :on-success="uploadKeySuccess">
                        <el-button type="primary">点击上传</el-button>
                        <template #tip>
                            <p class="text-red-500">
...
const getData = async () => {
    loading.value = true
    const res = await fetchData(sysconfigApi.getSysconfig, '', '获取系统配置')
    if (!res.error) {
        for (const k in form) {
            form[k] = res[k]
        }
...
const submit = async () => {
    loading.value = true
    const res = await fetchData(sysconfigApi.updateSysconfig, {
        ...form
    }, '修改系统配置')
